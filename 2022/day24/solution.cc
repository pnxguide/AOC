...
void solve_A()
{
  ...
  while (!bfs.empty()) {
    ...
    bfs.pop_front(); bfs_cnt.pop_front();
    ...
    if (cur_X == GOAL_X && cur_Y == GOAL_Y) {
      ...
      if (cnt+1 == max_cached) {
        ...
        for (int i = 0; i < ROW; i++) {
          for (int j = 0; j < COL; j++) {
            if (cached_u[cnt][i][j]) {
              if (i == 0) tmp_u[ROW-1][j] = true;
              else tmp_u[i-1][j] = true;
              // std::cout << "u" << std::endl;
            }

            if (cached_d[cnt][i][j]) {
              if (i == ROW-1) tmp_d[0][j] = true;
              else tmp_d[i+1][j] = true;
              // std::cout << "d" << std::endl;
            }

            if (cached_l[cnt][i][j]) {
              if (j == 0) tmp_l[i][COL-1] = true;
              else tmp_l[i][j-1] = true;
              // std::cout << "l" << std::endl;
            }

            if (cached_r[cnt][i][j]) {
              if (j == COL-1) tmp_r[i][0] = true;
              else tmp_r[i][j+1] = true;
              // std::cout << "r" << std::endl;
            }
          }
        }
      ...
      // try to move down
      if (cur_Y < ROW-1 && 
          !cached_d[cnt+1][cur_Y+1][cur_X] && 
          !cached_u[cnt+1][cur_Y+1][cur_X] && 
          !cached_l[cnt+1][cur_Y+1][cur_X] && 
          !cached_r[cnt+1][cur_Y+1][cur_X]) {
      ...
      if (cur_Y >= 0) {
        ...
        // try to move left
        if (cur_X > 0 && 
          !cached_d[cnt+1][cur_Y][cur_X-1] && 
          !cached_u[cnt+1][cur_Y][cur_X-1] && 
          !cached_l[cnt+1][cur_Y][cur_X-1] && 
          !cached_r[cnt+1][cur_Y][cur_X-1]) {
        ...
        // wait
        if (!cached_d[cnt+1][cur_Y][cur_X] && 
            !cached_u[cnt+1][cur_Y][cur_X] && 
            !cached_l[cnt+1][cur_Y][cur_X] && 
            !cached_r[cnt+1][cur_Y][cur_X]) {
